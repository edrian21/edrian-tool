...
# --- CONFIGURATION (DITO MO PALITAN ANG PASSWORD) ---

USERNAME = "Edrian"

PASSWORD = "Edrian@321" # Pwede mong palitan ito sa kahit anong gusto mo
...
def loading_anim(text="Loading"):
    ...
    for i in range(10):
        ...
        sys.stdout.write(f"\r{C}[*] {text} {animation[i % len(animation)]}{W}")
...
def login():
    ...
    os.system('figlet -f slant "SECURITY"')

    print(f"{W}==========================================")

    print(f"{R}      RESTRICTED ACCESS: MASTER ONLY       ")
    ...
    # Ang getpass ay hindi nagpapakita ng password habang tinitype

    p_input = getpass.getpass(f"{G}[+] Password: {W}")



    if u_input == USERNAME and p_input == PASSWORD:

        print(f"\n{G}[!] Access Granted! Welcome, Master {USERNAME}.{W}")

        time.sleep(1.5)

        loading_anim("Initializing Menu")

        menu()

    else:

        print(f"\n{R}[X] Access Denied! Maling Username o Password.{W}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        login() # Magsisimula ang tool sa Login
